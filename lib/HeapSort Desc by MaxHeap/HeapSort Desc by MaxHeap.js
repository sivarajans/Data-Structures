...
function HeapSortDesc(a) {
  var sorted = []; // first time to do max heap invarient for all parent nodes. floor(n/2) - 1.
  ...
  while (a.length > 0) {
    ...
    swap(a, 0, a.length - 1);
    sorted.push(a.pop()); // do heap invarient call for root node as it is replaced with last node.
...
function maxHeapIt(a, n, i) {
  var max = i;
  var left = Math.floor(i * 2) + 1; //left child

  var right = Math.floor(i * 2) + 2; //right child
